 ...
 *
 * <p>1、任何对正在进行的 RPC 的中止都被视为该 RPC 的“取消”。取消的常见原因包括客户端显式取消、截止时间过期和 I/O 失败。《服务无法得知取消的具体原因》。
 * <p>2、服务可以通过两个 API 被通知关于 RPC 取消的情况：io.grpc.Context 和 ServerCallStreamObserver
 */
public class CancellationServer {
  ...
  static class SlowEcho extends EchoGrpc.EchoImplBase {
    ...
    public void unaryEcho(EchoRequest request, StreamObserver<EchoResponse> responseObserver) {
      ...
      //如果在这个期间，context 被 cancel 了，那么就会调用 onError()。否则，走到正常返回
      // Let's start a multi-part operation. We can check cancellation periodically.
      //这里就是模拟服务端耗时操作。每次耗时操作之前判断下客户端是否取消了请求。如果请求就不继续执行下去了
      for (int i = 0; i < 10; i++) {
